    ...
    var body: some View {
        ZStack {
            ...
            if viewModel.showCelebration {
                LevelCompleteView(onBackToLevels: onBackToLevels, onQuiz: onQuiz)
                    .transition(.opacity)
            }
        }
    ...
    private var slidersSection: some View {
        ...
        return ScrollView(.vertical, showsIndicators: false) {
            VStack(spacing: 6) {
                ...
                if config.showM { MathSlider(label: config.mLabel, symbol: "m", value: $bindableVM.paramM, range: config.mRange, color: AppColors.accent, step: 0.1) }
                if config.showA { MathSlider(label: config.aLabel, symbol: "a", value: $bindableVM.paramA, range: config.aRange, color: AppColors.accentGold, step: 0.1) }
                if config.showB { MathSlider(label: config.bLabel, symbol: "b", value: $bindableVM.paramB, range: config.bRange, color: Color(red: 0.6, green: 0.4, blue: 1.0), step: 0.1) }
                if config.showC { MathSlider(label: config.cLabel, symbol: "c", value: $bindableVM.paramC, range: config.cRange, color: AppColors.accentPink, step: 0.1) }
                if config.showH { MathSlider(label: config.hLabel, symbol: "h", value: $bindableVM.paramH, range: config.hRange, color: Color(red: 0.3, green: 1.0, blue: 0.7), step: 0.1) }
                if config.showK { MathSlider(label: config.kLabel, symbol: "k", value: $bindableVM.paramK, range: config.kRange, color: Color(red: 1.0, green: 0.5, blue: 0.3), step: 0.1) }
            }
            .padding(.horizontal)
        }
        .frame(maxHeight: 170)
