...
private struct WelcomePage: View {
    @State private var marbleScale: CGFloat = 0.5
...
private struct RollAndCollectPage: View {
        ...
        VStack(spacing: 28) {
            ...
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    .position(x: 0, y: 0)
                    .modifier(MarblePathEffect(progress: marbleProgress, size: geo.size, starPositions: starPositions) { currentProgress in
                        for index in 0..<3 {
                            if currentProgress >= starPositions[index] - 0.02 && !starsCollected[index] {
            ...
            .offset(y: buttonAppear ? 0 : 20)
            .animation(.easeOut(duration: 0.6), value: buttonAppear)
    ...
    private func animateIn() {
        if starPulse { return } // Prevent multiple onAppear calls
    ...
    private func rollMarble() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            if currentToken == self.rollToken {
...
struct MarblePathEffect: GeometryEffect {
    var progress: CGFloat
    let size: CGSize
    let starPositions: [CGFloat]
    let onCollect: (CGFloat) -> Void
    ...
    func effectValue(size: CGSize) -> ProjectionTransform {
        let x = progress * self.size.width
        let y = self.size.height / 2 + CGFloat(sin(Double(progress) * .pi * 2) * 40)
